...
#error "Unknown Operating System!"
        ...
        KeyCodes ConvertToKeyCode(int key) {
            if (key <= 255) {
                return static_cast<KeyCodes>(NATIVE_TO_HID[e.Key]);
            }
        ...
        public:
            ...
            virtual std::shared_ptr<IInputConfiguration> onEvent(const std::function<void(const MouseEvent&)>& cb)  override {
                assert(!Impl->OnMouseEvent);
                Impl->OnMouseEvent = cb;
                return std::make_shared<InputConfiguration>(Impl);
            }
